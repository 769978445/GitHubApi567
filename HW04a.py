...
def get_repo_info(user_id='769978445'):  # Given a user <ID>
    output = []  # initial output
    user_url = 'https://api.github.com/users/{}/repos'.format(user_id)  # To retrieve a user's list of repositories
    res = requests.get(user_url)
    repos = json.loads(res.text)
    output.append('User: {}'.format(user_id))
